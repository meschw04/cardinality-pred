#
# Largely taken from mscn - util.py Modified slightly to suit our data structures
# Table Encoding and Join Encoding Modified
#
...
def encode_data(predicates, joins, column_min_max_vals, column2vec, op2vec, join2vec):
    ...
    for i, query in enumerate(predicates):
        ...
        if not predicate in joins[i]:
            join_vec = np.zeros(len(join2vec[joins[0][0]]),dtype=float)
